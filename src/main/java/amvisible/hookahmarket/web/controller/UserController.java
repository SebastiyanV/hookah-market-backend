...
public class UserController {
    ...
    @PostMapping(value = "/settings/update-profile-picture")
    @PreAuthorize("hasRole('ROLE_USER')")
    public ResponseEntity<?> updateProfilePicture(
        ...
        try {
            this.userService.updateProfilePicture(profilePicture, principal.getName());
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(new MessageResponseModel(THERE_IS_A_PROBLEM));
        }
        return ResponseEntity.ok().body(this.userService.getProfilePictureNameByEmail(principal.getName()));
    }

    @DeleteMapping(value = "/settings/delete-profile-picture")
    @PreAuthorize("hasRole('ROLE_USER')")
